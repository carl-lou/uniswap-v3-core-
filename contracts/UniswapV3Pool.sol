...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    int24 public immutable override tickSpacing; //很小的价格区间 池刻度间距
    ...
    // 储存一些 全局会用到的数据
    struct Slot0 {
        // the current price 当前价格,   token0/token1的值再开根号  就是sqrtPriceX96
        ...
        // tick采用1.0001等比数列,即 1，1.0001，1.0001*1.0001，1.0001^3，1.0001^4，也就是LP做市商老板 每个可选价格之间的差值为0.01%，相对还是比较细的
        int24 tick;
    ...
    /// @inheritdoc IUniswapV3PoolState
    // 当前可用于池的范围内的流动性,   liquidity等于x*y=k中K的平方根  ，liquidity*liquidity=常数k=x*y；
    // 存根号K而不是直接存K,是为了方便运算，以及减少溢出的可能性
    uint128 public override liquidity;

    /// @inheritdoc IUniswapV3PoolState
    // 查找池中特定刻度的信息,key是刻度数值，也就是价格。
    ...
    function observe(
        uint32[] calldata secondsAgos //多少秒之前。从多长时间以前，每个累计滴答和流动性价值应返回
    )
        external
        view
        override
    ...
    function _updatePosition(
        ...
        if (liquidityDelta != 0) {
            ...
            if (flippedLower) {
                // 若tickLower位置激活状态变化了，则 tickBitmap里也需要更新一下，wordPos和mask按位异或
    ...
    /// @inheritdoc IUniswapV3PoolActions
    // 无委托调用 通过_modifyPosition函数间接调用了
    /// @dev noDelegateCall is applied indirectly via _modifyPosition
    function mint(
        address recipient, //流动性拥有者
        int24 tickLower, //价格下限刻度的索引
        int24 tickUpper,
        uint128 amount, // 流动性 常数K的平方根
        bytes calldata data
