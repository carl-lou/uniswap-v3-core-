...
library SwapMath {
    ...
    function computeSwapStep(
        ...
        // 如果当前价格大于等于目标价，那说明是从token0兑换成token1，token0的价格降低，也就是token0兑换成token1（兑换过程中 数量要增加的币种会变得更泛滥，价格下降，毕竟物以稀为贵，越少越贵，越泛滥越便宜）
        // 比如token0，token1,原本池子里都有100个， 现在有一个第三方的人，用50个token0要兑换成token1，那么x的数量会上升，变成150个（这时候假设Y变成了50个）
        // ，那么x的价格为  y的数量/x的数量  = 50/150 = 1/3元，x的价格从1变成了1/3
        bool zeroForOne = sqrtRatioCurrentX96 >= sqrtRatioTargetX96;
        // 若tokenIn 的余额 >=0，表示还有余额还没被用完
        ...
        if (exactIn) {
            ...
            //如果注入的资金比 这次达到目标价消耗的资金量多，那么下一次的价格就是这次的目标价；也表示价格能够真的变化到目标价
